...
const Groups: React.FC = () => {
    ...
    const [isDeleteGroup, setDeleteGroup] = useState<boolean>(false);
        ...
        if (groups.some(group => group.id === currentId)) {
            setGroups(prevGroups =>
                prevGroups.map(group =>
                    group.id === currentId ? { ...group, school: name } : group
                )
            );
        } else {
                ...
                <div className="font-sans">
                                        ...
                                        <React.Fragment key={index}>
                                            <tr className={`${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} border-b`}>
                                                ...
                                                <td className="pl-10 py-4 flex">
                                                    <button className="flex justify-center" onClick={() => { openAddGroup(row.id); }}>
                                                        <Image src={EditIcon} alt="Edit" className="mx-auto w-10 h-10" />
                                                    </button>
                                                    <button className="flex justify-center" onClick={() => { openDeleteGroup(row.id, row.school); }}>
                    ...
                    <div className="absolute bottom-0 left-0 w-full border-t border-gray-300 bg-gray-50 px-12 py-4">
                        <div className="flex justify-between">
                            <button onClick={() => openAddGroup(nextId)} className="text-1xl font-bold underline px-4" style={{ color: '#006330' }}>
                                Add a school
                            </button>
                            <button className="rounded-full text-1xl px-6 py-3 text-white" style={{ backgroundColor: '#006330' }}>
                                Save Changes
                            </button>
                        </div>
                    </div>
